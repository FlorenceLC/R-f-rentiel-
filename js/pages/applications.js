...
export function renderApplications(container) {
  ...
  container.innerHTML = `
    ...
    ${sectionTitle('🟢', `${t('apps.title')} (${apps.length})`)}
    ${apps.length ? `
          ...
          <div class="card" style="cursor:pointer" onclick="navigate('catalogue',{cu_id:'${app.cu_id}'})">
            <div style="display:flex;align-items:flex-start;gap:12px;">
              ...
              <div style="flex:1">
                ...
                ${app.origine ? `<div class="text-sm" style="color:var(--text-muted);margin-top:2px;">${t('apps.origin')} : ${originLabel(app.origine)}</div>` : ''}
                ...
                ${app.application_url ? `<a href="${app.application_url}" target="_blank" class="btn btn-primary btn-sm mt-2" onclick="event.stopPropagation()">${t('apps.access')} →</a>` : ''}
              </div>
            </div>
          </div>`).join('')}
      </div>` : emptyState('🔍', t('apps.empty'), '')}

    ${all_prod.length ? `
      ${sectionTitle('📋', `${t('home.kpi.production')} (${all_prod.length})`)}
        ...
        <table class="data-table">
          ...
          <tbody>
            ${all_prod.map(cu => `<tr style="cursor:pointer" onclick="navigate('catalogue',{cu_id:'${cu.cu_id}'})">
              ...
              <td><button class="btn btn-ghost btn-sm" onclick="event.stopPropagation();navigate('guide')">${t('guide.contact.btn')}</button></td>
            </tr>`).join('')}
          </tbody>
        </table>
      </div>` : ''}
